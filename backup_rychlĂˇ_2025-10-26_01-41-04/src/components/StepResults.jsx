...
const StepResults = ({ navrhy, vybraneIndikatory, onNext, onBack, setNavrhy, vahy = {}, categoryWeights, aiWeights = null, aiCategoryWeights = null }) => {
  ...
  // Synchronizace s globálním stavem váh
    ...
    return zpracovaneNavrhy.map(navrh => {
      ...
      indicators.forEach(indikator => {
        ...
        if (actualValue !== null && actualValue !== undefined) {
          ...
          console.log(`📊 Počet validních indikátorů pro ${navrh.nazev}:`, filledIndicators + 1);
          filledIndicators++;
  ...
  }, [navrhy, indicators, globalWeights]);
  ...
  const getNejlepsiHodnota = (indikatorId) => {
    ...
    // Debug pre konkrétne indikátory len v development režime
    if (import.meta.env.DEV && (indikatorId === 'H02' || indikatorId === 'Q01')) {
      console.log(`🎯 getNejlepsiHodnota pro ${indikatorId} (${indikator.nazev}):`, {
        hodnoty: hodnoty,
        lower_better: indikator.lower_better,
        nejlepsi: hodnoty.length > 0 ? (indikator.lower_better ? Math.min(...hodnoty) : Math.max(...hodnoty)) : null,
        scoredProposals: scoredProposals.map(p => ({ id: p.id, nazev: p.nazev, data: p.data[indikatorId] }))
      });
    }
  ...
  const isNejlepsiHodnota = (navrh, indikatorId) => {
    ...
    const isNejlepsi = nejlepsiHodnota !== null && actualNavrhHodnota === nejlepsiHodnota;
    
    // Debug pre konkrétne indikátory len v development režime
    if (import.meta.env.DEV && (indikatorId === 'H02' || indikatorId === 'Q01')) {
      console.log(`🎯 isNejlepsiHodnota pro ${indikatorId} v návrhu ${navrh.nazev}:`, {
        nejlepsiHodnota: nejlepsiHodnota,
        actualNavrhHodnota: actualNavrhHodnota,
        isNejlepsi: isNejlepsi,
        navrhHodnota: navrhHodnota
      });
    }
    ...
    <div className="card-active overflow-hidden animate-fade-in">
            ...
            <tbody>
              {scoredProposals.map(navrh => (
                <tr key={navrh.id} className="table-row">
                  ...
                  {vybraneIndikatoryList.map(indikator => {
                    const hodnota = navrh.data[indikator.id];
                    // Debug logy len v development režime
                    ...
                    // Debug pre konkrétne indikátory len v development režime
                    if (import.meta.env.DEV && (indikator.id === 'H02' || indikator.id === 'Q01')) {
                      console.log(`🎯 DEBUG ${indikator.id} (${indikator.nazev}) v návrhu ${navrh.nazev}:`, {
                        actualHodnota: actualHodnota,
                        isNejlepsi: isNejlepsi,
                        hodnota: hodnota
                      });
                    }
      ...
      {showWinnerBreakdown && (
        <WinnerCalculationBreakdown
          ...
          globalCategoryWeights={globalCategoryWeights}
      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {Object.entries(indicatorsByCategory).map(([categoryId, categoryIndicators]) => {
            ...
            const categoryInfo = kategorie.find(k => k.nazev === categoryId);
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {categoryIndicators.map(indicator => {
                        ...
                        // Debug: zobraz source pre každý indikátor
                        console.log(`🔍 Indikátor ${indicator.id} (${indicator.nazev}):`, {
                          value: value,
                          actualValue: actualValue,
                          source: value && typeof value === 'object' ? value.source : 'undefined'
                        });
                        ...
                        return (
                          <tr key={indicator.id} className="border-t border-gray-200 hover:bg-gray-50">
                            <td className="px-4 py-3">
                              ...
                              <div className="text-xs text-gray-500 mb-1">{indicator.jednotka}</div>
                              {indicator.description && (
                                <div className="text-xs text-gray-600 mb-1" title={indicator.description}>
                                  {indicator.description}
                                </div>
                              )}
                              <div className="flex items-center gap-1 text-xs">
                                <span className={`px-1 py-0.5 rounded ${
                                  indicator.trend === 'up' ? 'text-green-600 bg-green-100' :
                                  indicator.trend === 'down' ? 'text-red-600 bg-red-100' :
                                  'text-gray-600 bg-gray-100'
                                }`}>
                                  {indicator.trend === 'up' ? '📈' : indicator.trend === 'down' ? '📉' : '➡️'}
                                </span>
                                <span className="text-purple-600">Váha: {indicator.vaha || 1}/5</span>
                                <span className="text-blue-600">{indicator.typ || 'N/A'}</span>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-center font-mono text-sm">
                              <div className="flex items-center justify-center gap-1">
                                {formatValue(actualValue, indicator.jednotka)}
                                {proposal.data[indicator.id] && typeof proposal.data[indicator.id] === 'object' && proposal.data[indicator.id].source === "uživatelský vstup" && (
...
const WinnerCalculationBreakdown = ({ 
  ...
  }, [vybraneNavrhyData, vybraneIndikatory, globalWeights, globalCategoryWeights, sortBy, sortOrder, vybraneIndikatoryList]);
