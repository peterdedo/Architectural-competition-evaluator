...
const ComparisonDashboard = ({ 
  ...
  const getValueColor = (navrh, indikatorId) => {
    const isBest = isNejlepsiHodnota(navrh, indikatorId);
    if (isBest) return 'bg-green-50 border-l-4 border-green-500';
    return 'bg-white';
  };
                      ...
                      <tr key={indikator.id} className="border-t border-gray-200 hover:bg-gray-50 transition-colors">
                        <td className="p-4">
                          <div>
                            ...
                            <div className="text-sm text-gray-500 mt-1">Popis indikátoru</div>
                          </div>
                        </td>
                        ...
                        {navrhyWithScores.map((navrh) => {
                          ...
                          return (
                            <td key={navrh.id} className={`text-center p-4 ${getValueColor(navrh, indikator.id)}`}>
                              {actualValue != null && actualValue !== '' ? (
                                <div className="flex items-center justify-center gap-2">
                                  <span className="font-mono text-sm">{formatHodnota(actualValue, indikator.jednotka)}</span>
                                  {isBest && <span className="text-green-600 text-sm">✓</span>}
                                </div>
                              ) : (
          ...
          {zobrazeni === 'radar' && vybraneNavrhyData.length > 0 && (
            ...
            >
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Radarový graf</h3>
                <p className="text-gray-500">Radarový graf bude implementovaný v budúcej verzii.</p>
              </div>
            </motion.div>
          )}
          ...
          {zobrazeni === 'dashboard' && vybraneNavrhyData.length > 0 && (
            ...
            >
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Dashboard prehľad</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {navrhyWithScores.map((navrh) => (
                    <div key={navrh.id} className="bg-gray-50 rounded-xl p-4">
