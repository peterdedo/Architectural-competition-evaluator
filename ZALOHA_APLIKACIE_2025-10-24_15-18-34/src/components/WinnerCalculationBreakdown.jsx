...
const WinnerCalculationBreakdown = ({ 
  ...
  const navrhyWithScores = useMemo(() => {
    ...
    try {
      ...
      // Použij normalizované skóre z WizardContext
      const navrhyWithDetails = filteredResults.map(project => {
        // Použij normalizované skóre pokud existuje, jinak weightedScore
        const displayScore = project.normalizedScore || project.weightedScore || 0;
        const originalScore = project.weightedScore || 0;
        
        // Vytvor detailné skóre pre zobrazenie z indicatorScores
        const detailedScores = [];
        if (project.indicatorScores && Array.isArray(project.indicatorScores)) {
          project.indicatorScores.forEach(score => {
            const indikator = vybraneIndikatoryList.find(ind => ind.id === score.id);
            if (indikator) {
              detailedScores.push({
                id: score.id,
                ...
                hodnota: score.value,
                weight: score.weight,
                categoryWeight: score.categoryWeight,
                finalWeight: (score.weight * score.categoryWeight) / 100,
                normalizedValue: score.normalizedValue,
                weightedValue: score.weightedScore,
        ...
        return {
          ...
          totalWeight: Object.values(standardizedWeights).reduce((sum, cat) => 
            sum + (cat.weight || 0), 0
          ),
          weightedScore: displayScore,
          originalScore: originalScore,
          completionRate: Math.round((detailedScores.length / vybraneIndikatoryList.length) * 100),
          filledIndicators: detailedScores.length
        };
      });
