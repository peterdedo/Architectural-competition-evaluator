...
import { shouldExcludeCustomIndicator } from '../config/legacyIndicatorFilters';
...
export const getAllIndicators = () => {
  ...
  const filteredCustomIndicators = customIndicators.filter(
    (indicator) => !shouldExcludeCustomIndicator(indicator)
  );
  
  return [...indikatory, ...filteredCustomIndicators];
};
