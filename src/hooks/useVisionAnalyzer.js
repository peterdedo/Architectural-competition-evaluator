...
export const useVisionAnalyzer = () => {
  ...
  // Validácia JSON odpovede od OpenAI s bezpečným parsovaním
  ...
  const realAnalyze = useCallback(async (project, criteria, model = 'gpt-4o') => {
...
Vrať POUZE JSON objekt ve formátu:
{
  "název_kritéria": {
    ...
    // Bezpečné parsovanie JSON s error handling a fallback
    let parsedData;
    try {
      ...
      try {
        ...
        parsedData = JSON.parse(corrected);
        console.log('✅ JSON opravený pomocí fallback mechanizmu');
      } catch (fallbackError) {
        console.error('Fallback JSON parse error:', fallbackError);
        throw new Error('Neplatný JSON formát od OpenAI API - ani fallback nefungoval');
      }
    }
  ...
  const analyze = useCallback(async (project, criteria, model = 'gpt-4o', useMock = false) => {
    setIsAnalyzing(true);
    setError(null);
    ...
    try {
      ...
      if (useMock) {
        ...
        result = await realAnalyze(project, criteria, model);
      }
