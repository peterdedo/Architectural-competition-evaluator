...
// Rychlejší čtení hmoty stavby než tabulka čísel – kolik podlaží, jak jsou rozložená.
    ...
    <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(9rem, 1fr))' }}>
      {items.map((p) => (
        <div key={p.id} className="text-center">
          <div className="text-xs font-semibold text-text-light truncate mb-2" title={p.nazev}>{p.nazev}</div>
          <div className="flex items-end justify-center gap-1.5" style={{ height: MAX_BAR_H }}>
            {p.floors.map((f, i) => (
              <div key={i} className="flex flex-col items-center gap-1 w-9">
                <div
                  ...
                  style={{
                    ...
                    background: f.value ? p.color : '#E2E8F0', // odpovídá tailwind.config.js colors.border
                    opacity: f.value ? 1 : 0.4,
                  }}
                  title={f.value ? `${f.label}: ${f.value.toLocaleString('cs-CZ')} m²` : `${f.label}: —`}
                />
                <span className="font-mono text-[0.6rem] text-text-muted block w-full truncate text-center" title={f.label}>
