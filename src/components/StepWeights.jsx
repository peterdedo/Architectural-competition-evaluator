...
const StepWeights = ({ isOpen, onClose, onSave, selectedIndicators, vahy, setVahy, categoryWeights: globalCategoryWeights, setCategoryWeights: setGlobalCategoryWeights }) => {
  ...
  // AI Weight Manager states (používa sa samostatná komponenta)
  
  // Wizard context
  const { updateWeights, weights: wizardWeights, categoryWeights: wizardCategoryWeights } = useWizard();
  const { showToast } = useToast();
  
  // AI Weight Manager funkce sa používa v samostatnej komponente
  ...
  useEffect(() => {
    if (wizardWeights && Object.keys(wizardWeights).length > 0) {
      ...
      console.log('🔍 StepWeights - wizardWeights:', wizardWeights);
  ...
  // Duplicitný useEffect pre vahy prop bol odstránený - používa sa wizardWeights

  // Duplicitný useEffect pre wizardWeights bol odstránený
