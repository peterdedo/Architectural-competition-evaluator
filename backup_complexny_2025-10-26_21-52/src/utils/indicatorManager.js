...
export const getAllIndicators = () => {
  ...
  // Automaticky odstráň "Toalety" a podobné indikátory
  const filteredCustomIndicators = customIndicators.filter(indicator => {
    const nazev = indicator.nazev ? indicator.nazev.toLowerCase() : '';
    return !nazev.includes('toalety') && 
           !nazev.includes('toilet') && 
           !nazev.includes('wc') &&
           indicator.id !== 'custom_1761333530207'; // Konkrétne ID "Toalety"
  });
