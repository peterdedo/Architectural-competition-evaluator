...
const WeightedHeatmap = ({ vybraneNavrhyData, vybraneIndikatoryList, vahy = {}, categoryWeights = {} }) => {
  ...
  const option = useMemo(() => {
    ...
    xAxis: {
      type: 'category',
      data: vybraneIndikatoryList.filter(ind => ind.id !== 'custom_1761333530207').map(i => i.nazev),
      axisLabel: {
        rotate: 45,
        fontSize: 12,
        color: '#666'
    ...
    visualMap: {
      min: 0,
      max: (() => {
        const maxValueForMap = Math.max(3000, ...heatmapData.map(d => Number(d[2]) || 0));
        return isFinite(maxValueForMap) ? maxValueForMap : 3000;
      })(),
      calculable: true,
      orient: 'vertical',
      right: 10,
