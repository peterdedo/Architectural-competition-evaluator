...
const AdvancedAIAssistant = ({ 
  ...
  // Získanie results z WizardContext
  ...
  // Pridanie skóre k návrhom z results
  const vybraneNavrhyData = navrhy.filter(navrh => vybraneNavrhy.has(navrh.id)).map(navrh => {
    const result = results?.find(r => r.id === navrh.id);
    return {
      ...navrh,
      weightedScore: result?.weightedScore || 0,
      completionRate: result?.completionRate || 0,
      filledIndicators: result?.filledIndicators || 0,
      totalIndicators: result?.totalIndicators || 0
    };
  });
