...
const StepCriteria = ({ vybraneIndikatory, setVybraneIndikatory, onNext, onBack, vahy, setVahy, categoryWeights, setCategoryWeights }) => {
  ...
  useEffect(() => {
    ...
    if (Object.keys(globalWeights).length > 0 && JSON.stringify(globalWeights) !== JSON.stringify(vahy)) {
      console.log("🔄 Synchronizuji váhy indikátorů z globálního stavu...");
    ...
    if (Object.keys(globalCategoryWeights).length > 0 && JSON.stringify(globalCategoryWeights) !== JSON.stringify(categoryWeights)) {
      console.log("🔄 Synchronizuji váhy kategorií z globálního stavu...");
  ...
  const refreshWeightsFromContext = useCallback(() => {
    if (Object.keys(globalWeights).length > 0) {
      setVahy(globalWeights);
      console.log("✅ Váhy indikátorů aktualizovány z kontextu");
    }
    if (Object.keys(globalCategoryWeights).length > 0) {
      setCategoryWeights(globalCategoryWeights);
      console.log("✅ Váhy kategorií aktualizovány z kontextu");
    }
  }, [globalWeights, globalCategoryWeights, setVahy, setCategoryWeights]);
  ...
  const handleWeightChange = useCallback((indicatorId, newWeight) => {
    ...
    try {
      const wizardContext = useWizard();
      if (wizardContext.updateWeights) {
        wizardContext.updateWeights({
  ...
  // Automatické prepísanie AI váh do lokálneho stavu
  useEffect(() => {
    if (globalWeights && Object.keys(globalWeights).length > 0) {
      console.log('✅ Váhy indikátorů aktualizovány z kontextu');
      setVahy(prev => ({ ...prev, ...globalWeights }));
    }
  }, [globalWeights, setVahy]);

  useEffect(() => {
    if (globalCategoryWeights && Object.keys(globalCategoryWeights).length > 0) {
      console.log('✅ Váhy kategorií aktualizovány z kontextu');
      setCategoryWeights(prev => ({ ...prev, ...globalCategoryWeights }));
    }
  }, [globalCategoryWeights, setCategoryWeights]);
  ...
  const handleVybratVseVKategorii = (kategorieNazev) => {
    const kategorieIndikatory = indicators
      .filter(indicator => indicator.kategorie === kategorieNazev)
  ...
  // Helper functions
  const getPocetVybranychVKategorii = (kategorieNazev) => {
    const kategorieIndikatory = indicators
      .filter(indicator => indicator.kategorie === kategorieNazev)
      .map(indicator => indicator.id);
        ...
        <div className="mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            >
                ...
                <button 
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg font-semibold hover:shadow-lg transition-all shadow-sm hover:shadow-md"
        ...
        <div className="space-y-4">
          {kategorie.map((kategorie) => {
            const kategorieIndikatory = filteredIndicators.filter(indicator => 
              indicator.kategorie === kategorie.nazev
            );
            
            if (kategorieIndikatory.length === 0) return null;
            
            const isExpanded = expandedCategories.has(kategorie.id);
            const vybranoVKategorii = getPocetVybranychVKategorii(kategorie.nazev);
              ...
              >
                ...
                >
                  ...
                  <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleVybratVseVKategorii(kategorie.nazev);
                      }}
                      style={{
