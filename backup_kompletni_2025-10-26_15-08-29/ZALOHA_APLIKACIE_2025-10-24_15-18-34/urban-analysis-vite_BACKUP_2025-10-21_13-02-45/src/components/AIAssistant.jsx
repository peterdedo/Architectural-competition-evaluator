              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'comments' && (
                  <motion.div
                    ...
                    className="space-y-6"
                  >
                    <h3 className="text-xl font-semibold text-gray-900">Komentáře k návrhům</h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {data.navrhy?.map((navrh) => (
                        <div key={navrh.id} className="bg-white border border-gray-200 rounded-xl p-4">
                          <div className="flex items-center justify-between mb-3">
                            <h4 className="font-semibold text-gray-900">{navrh.nazev}</h4>
                            <span className="text-sm text-gray-500">{navrh.weightedScore || 0}%</span>
                          </div>
                          <motion.button
                            onClick={() => handleGenerateComment(navrh)}
                            disabled={isAnalyzing}
