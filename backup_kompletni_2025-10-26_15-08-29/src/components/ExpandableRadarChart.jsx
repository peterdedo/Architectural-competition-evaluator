    ...
    return (
      ...
      >
        ...
        <div className="h-80 w-full" ref={chartRef}>
          <RadarChartAdvanced
            data={cleanedData}
            indicators={(indicators || []).filter(ind => ind.id !== 'custom_1761333530207')}
          ...
          <div className={`flex-1 overflow-auto bg-gray-50 p-6 md:p-8 flex flex-col`}>
            ...
            >
              ...
              <div className="bg-white rounded-lg p-4 border border-green-200 shadow-sm">
                ...
                <p className="text-2xl font-bold text-[#4BB349]">{(indicators || []).filter(ind => ind.id !== 'custom_1761333530207').length}</p>
              </div>
              <div className="bg-white rounded-lg p-4 border border-purple-200 shadow-sm">
                <p className="text-sm text-gray-600 mb-1">Datových bodů</p>
                <p className="text-2xl font-bold text-purple-600">{(cleanedData?.length || 0) * ((indicators || []).filter(ind => ind.id !== 'custom_1761333530207').length)}</p>
              </div>
